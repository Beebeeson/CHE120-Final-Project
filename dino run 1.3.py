...
class Dinosaur:
    def __init__(self):
        self.width = 48
        self.height = 50 #1.3: modified collision box to fit sprite
        self.x = 50
        self.y = HEIGHT - self.height - 20 # 1.3: adjusted constant from 10 to 20 to keep dinosour a bit away from bottom of the window after asjusting collision box
        self.vel_y = 0
    ...
    def update(self):
        if self.is_jumping:
            self.vel_y += GRAVITY
            self.y += self.vel_y
            if self.y >= HEIGHT - self.height - 20:  # Ensure the dinosaur is at above the ground
...
class Obstacle:
    def __init__(self):
        self.width = 20
        self.height = random.randint(40, 100)  # Random height for obstacles, can go higher
        self.x = WIDTH# 1.3: raised maxinum range of height to 100 to increase difficulty
        self.y = HEIGHT - self.height - 15 #1.3: adjusted obstacle starting height to match ground layer (aesthetically)
    ...
    def update(self):
        ...
        if self.x < 0:
            self.x = WIDTH
            self.height = random.randint(40, 100)  # Reset height for new obstacles
...
def main():
    ...
    score = 0
    run = True
    global scroll #made scroll into global reference to keep scrolling factor working thought the run
    ...
    while run:
        ...
        #1.3: tip text in-game for double jumping
        font = pygame.font.SysFont('Comic Sans',20)
        tips = font.render("(press space in midair to double-jump! Isn't that cool?!)", True, (200,200,25))
        screen.blit(tips, (45,40))
